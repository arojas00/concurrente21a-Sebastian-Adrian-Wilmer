...
void Bosque::changeForest(Map* map, Map* new_map, int numero_hilos) {
 ...
 #pragma omp parallel for num_threads(thread_count) default(none) shared(numero_filas) shared(mapping) shared(block_size) shared(numero_columnas) shared(matrix) shared(newMatrix) schedule(static, block_size)
